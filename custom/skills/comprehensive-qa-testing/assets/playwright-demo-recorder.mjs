/**
 * Generic client-presentable demo recorder (Playwright). Project-agnostic.
 *
 * WHY THIS EXISTS
 * Cypress and the Playwright MCP record only the page viewport, NOT the OS mouse
 * cursor — the result looks like a "preview", not a demo. This recorder drives the
 * REAL running app with Playwright, injects an animated cursor + a step-caption bar,
 * moves the mouse in visible steps, records the session to .webm, and (if available)
 * transcodes to a client-friendly .mp4 via ffmpeg-static.
 *
 ...
 *   WORK=$(mktemp -d)            # e.g. /tmp/tmp.XXturned into a scratch dir
 *   cd "$WORK"
 ...
 *     node /path/to/playwright-demo-recorder.mjs \
 *       --base http://localhost:3000 \
 *       --flow /path/to/your-flow.mjs \
 *       --out  ./qa-demo-output \
 *       --name login-demo
 *
 * Then copy <out>/<name>.mp4 into docs/qa-artifacts/<timestamp>_<feature>/.
 *
 ...
 *   --out  <dir>     Output directory                   (env DEMO_OUT;  default ./qa-demo-output)
 ...
 *   `h` provides: h.BASE, h.goto(pathOrUrl), h.caption(text), h.glideClick(locator, opts),
 *                 h.glideTo(locator, opts), h.type(locator, text), h.sleep(ms).
 */
...
// not from this script's folder — so the recorder can live anywhere (e.g. a skill dir)
// while deps live in the isolated $WORK dir. Anchor on the working dir, overridable.
...
const CONFIG = {
  ...
  OUT_DIR: path.resolve(args.out || process.env.DEMO_OUT || './qa-demo-output'),
...
// ── Injected per-document overlay: animated cursor + caption bar ──────────────
